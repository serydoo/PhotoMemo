...
struct MemoryConfigurationRecord:
    ...
    struct Editor:
        ...
        private struct PresentationStyleTemplateBinding:
            Codable {

            let style: RecordCardPresentationStyle
        ...
        /// Templates are isolated by presentation style. The legacy
        /// `template` field remains as the compatibility value for older
        /// configuration files that predate style-specific content.
        var templatesByPresentationStyle:
        ...
        var effectiveTemplatesByPresentationStyle:
            [RecordCardPresentationStyle: Template] {
            Dictionary(
                uniqueKeysWithValues:
                    RecordCardPresentationStyle.allCases.map { style in
                        (style, template(for: style))
                    }
            )
        }
        ...
        init(from decoder: Decoder) throws {
            ...
            let styleBindings = try container.decodeIfPresent(
                [PresentationStyleTemplateBinding].self,
                forKey: .templatesByPresentationStyle
            ) ?? []
            var seenStyles: Set<RecordCardPresentationStyle> = []
            if let duplicate = styleBindings.first(where: {
                !seenStyles.insert($0.style).inserted
            }) {
