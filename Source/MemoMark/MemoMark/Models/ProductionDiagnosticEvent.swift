        ...
        return Self(
            ...
            regionMetrics:
                activeConfiguration.map {
                    regionMetrics(
                        for: $0.editor.templatesByPresentationStyle[
                            $0.presentation.route
                        ] ?? $0.editor.template
                    )
                } ?? []
        )
...
nonisolated enum ProductionDiagnosticFailureClassifier {
    ...
    static func configurationSave(
        ...
        } else if let photoMemoError = error as? MemoMarkError {
...
#if !MEMOMARK_SHARE_EXTENSION
        if let photoMemoError = error as? MemoMarkError {
