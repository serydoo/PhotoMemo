...
nonisolated final class PhotoMetadataReader {
...
private nonisolated extension PhotoMetadataReader {
    ...
    private func parseDate(
        ...
        for format in Self.dateFormats {
            // DateFormatter is mutable and is not safe to share between
            // concurrent metadata reads. Create one per parse attempt so a
            // burst of shared-extension or batch work cannot race on parser
            // state.
            let formatter = Self.makeDateFormatter(format)
            if let date =
                formatter.date(
                    from: trimmedValue
                ) {
