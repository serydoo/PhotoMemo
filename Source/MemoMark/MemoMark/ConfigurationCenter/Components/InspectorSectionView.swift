...
enum ConfigurationUI {
    ...
    static let groupedCornerRadius = MemoMarkDesignTokens.CornerRadius.grouped
    ...
    static let groupedCornerRadius: CGFloat = 16
...
struct V1GroupedSurface: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .background(
                RoundedRectangle(
                    cornerRadius: ConfigurationUI.groupedCornerRadius,
                    style: .continuous
                )
                .fill(background)
            )
