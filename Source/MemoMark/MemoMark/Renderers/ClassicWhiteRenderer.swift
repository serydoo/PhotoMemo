...
struct ClassicWhiteCardRenderer: View {
    ...
    var body: some View {
        ...
        GeometryReader { geometry in
            ...
            VStack(spacing: 0) {
                ...
                ClassicWhiteInformationBarRenderer(
                    card: card,
                    width: geometry.size.width,
    ...
    static func informationBar(
        for card: RecordCard,
        width: CGFloat,
        height: CGFloat
    ) -> some View {
        let renderer = ClassicWhiteCardRenderer(
            image: Image(systemName: "circle"),
            card: card
        )
        return renderer.infoBar(
            width: width,
            height: height,
            layout: ClassicWhiteRenderer.layout(
                for: ClassicWhiteRenderer.orientation(
                    for: card.metadata
                )
            )
        )
...
/// The Classic White footer is rendered as an independent layer so the still
/// image and both resources of a Live Photo consume identical pixels and
/// geometry. The source photo is owned by the media compositor.
struct ClassicWhiteInformationBarRenderer: View {
    ...
    var body: some View {
        ClassicWhiteCardRenderer.informationBar(
            for: card,
            width: width,
            height: height
        )
