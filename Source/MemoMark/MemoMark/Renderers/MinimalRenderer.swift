...
enum MinimalRenderer {
    ...
    ///
    /// Minimal renders only the text areas declared by its style contract.
    /// This intentionally does not fall back to another style's slot: the
    /// editor, preview, still export, Live Photo overlay, and photo
    /// description must all observe the same independent content surface.
    static func informationText(for card: RecordCard) -> String {
        let blocks = CardTextBlockEngine().build(from: card)
        let renderedAreas = card.presentationStyle
            .contentContract
            .renderedTextAreas

        for area in renderedAreas {
            if let value = blocks
