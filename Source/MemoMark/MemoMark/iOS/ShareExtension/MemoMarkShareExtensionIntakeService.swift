...
struct MemoMarkShareExtensionError:
    Error {
    ...
    init(
        kind: Kind,
        importResult:
            MemoMarkShareExtensionImportResult? = nil,
        failureContext:
            MemoMarkShareIntakeFailureContext? = nil
    ) {
        self.kind = kind
...
@MainActor
final class MemoMarkShareExtensionIntakeService {
    ...
    ) async throws -> MemoMarkShareExtensionImportResult {

        let requestID = UUID()

        let handoffReadiness =
            MemoMarkSharedContainer.handoffReadiness()
        _ = MemoMarkShareDiagnostics.recordResult(
            stage: .appSharedContainerReadiness,
            message: handoffReadiness.diagnosticMessage,
            requestID: requestID,
            defaults: MemoMarkSharedContainer.sharedUserDefaults
        )
        guard handoffReadiness.canAttemptHandoff else {
            let failureContext =
                MemoMarkShareIntakeOperationSeed(
                    itemProviderCount: items
                        .flatMap { $0.attachments ?? [] }
                        .count,
                    supportedProviderCount: 0,
                    requestedTypeIdentifier: UTType.image.identifier
                )
                .failureContext(
                    stage: .persist,
                    operation: "persistSharedItems.sharedContainerReadiness",
                    error:
                        MemoMarkShareIntakeDiagnosticError.make(
                            description:
                                handoffReadiness.diagnosticMessage,
                            code: 2004
                        )
                )
            throw MemoMarkShareExtensionError(
                kind: .persistFailed,
