...
@MainActor
final class MemoryCardTextKitCommandBus {
    var insertHandler: ((MemoryCardContentItem) -> Void)?
    var focusHandler: (() -> Void)?
    var prefersTrailingInsertion = false
    private(set) var commandRevision = 0

    func insert(_ item: MemoryCardContentItem) {
        commandRevision += 1
        MemoryCardTextKitTrace.log("commandBus.insert", extra: "revision=\(commandRevision) item=\(item.kind)")
        insertHandler?(item)
    }

    func requestFocus() {
        MemoryCardTextKitTrace.log("commandBus.requestFocus")
        focusHandler?()
    }
}

private enum MemoryCardTextKitSelectionOwner: String {
    case uikit
    case userTouch
    case swiftUIUpdate
    case commandBus
...
private enum MemoryCardTextKitTrace {
...
private enum MemoryCardEditorPasteboard {
...
private struct MemoryCardTextKitUndoSnapshot {
...
enum MemoryCardEditorInputMetrics {
    ...
    static func attachmentBaselineOffset(
        ...
        MemoryCardEditorLineBoxGeometry.attachmentOriginY(
            lineHeight: lineHeight(for: font),
...
enum MemoryCardEditorCapsuleMetrics {
...
@MainActor
final class MemoryCardTextKitEditorSession: NSObject, UITextViewDelegate {
    ...
    private(set) var draft: MemoryCardEditorDraft
    ...
    private var selectionOwner: MemoryCardTextKitSelectionOwner = .uikit
    ...
    let onDraftChange: (MemoryCardEditorDraft) -> Void
    let onFocus: () -> Void
    ...
    init(
        ...
        region: CardRegion,
        onDraftChange: @escaping (MemoryCardEditorDraft) -> Void,
    ...
    func attach(to textView: UITextView) {
        ...
        textView.textContainerInset = UIEdgeInsets(
            top: 0,
            left: MemoryCardEditorInputMetrics.textContainerHorizontalInset,
            bottom: 0,
            right: MemoryCardEditorInputMetrics.textContainerHorizontalInset
        )
        ...
        MemoryCardTextKitTrace.log("attach", extra: stateDescription(for: textView))
        applyDraft(draft, to: textView, selection: selectedRange)
    }

    func setDraftFromOutside(_ nextDraft: MemoryCardEditorDraft, in textView: UITextView) {
        ...
        MemoryCardTextKitTrace.log("setDraftFromOutside", extra: stateDescription(for: textView))
        draft = nextDraft
        draftRevision += 1
    ...
    func copySelection(in textView: UITextView) -> Bool {
        ...
        guard !items.isEmpty,
              let data = MemoryCardEditorClipboardCodec.encode(
                MemoryCardEditorClipboardPayload(items: items)
              ) else {
            return false
        }

        let plainText = MemoryCardEditorClipboardPayload(items: items).displayText
        UIPasteboard.general.setItems(
            [[
                MemoryCardEditorPasteboard.structuredType: data,
                MemoryCardEditorPasteboard.legacyStructuredType: data,
                MemoryCardEditorPasteboard.plainTextType: plainText
            ]],
    ...
    func selectTrailingPosition(in textView: UITextView) {
        ...
        MemoryCardTextKitTrace.log("selectTrailingPosition", extra: stateDescription(for: textView))
    }
    ...
    func prepareForDeleteBackward(in textView: UITextView) {
        ...
        selectedRange = NSRange(location: documentLength, length: 0)
        textView.selectedRange = selectedRange
        MemoryCardTextKitTrace.log("prepareForDeleteBackward", extra: stateDescription(for: textView))
    }
    ...
    func deleteBackwardIfTrailingAttachment(in textView: UITextView) -> Bool {
        ...
        isApplyingExternalState = false
        emitDraftChangeIfNeeded()
        MemoryCardTextKitTrace.log("deleteAttachmentAtCaret", extra: stateDescription(for: textView))
        ...
        DispatchQueue.main.async { [weak self, weak textView] in
            ...
            MemoryCardTextKitTrace.log("restoreCaretAfterDelete.layout", extra: self.stateDescription(for: textView))
        }
        return true
    }
    ...
    func textViewDidChangeSelection(_ textView: UITextView) {
        ...
        MemoryCardTextKitTrace.log("selectionChanged", extra: stateDescription(for: textView) + " owner=\(selectionOwner.rawValue)")
    }
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if shouldRestoreTrailingCaretAfterDelete {
            ...
            MemoryCardTextKitTrace.log("restoreCaretAfterDelete", extra: stateDescription(for: textView))
        ...
        draft = projectedDraft(from: textView)
        draftRevision += 1
        MemoryCardTextKitTrace.log("textViewDidChange", extra: "draftRevision=\(draftRevision) " + stateDescription(for: textView))
        emitDraftChangeIfNeeded()
    }
    ...
    private func applyDraft(
        ...
        refreshTypingAttributes(in: textView)
        MemoryCardTextKitTrace.log("applyDraft", extra: "textStorageRevision=\(textStorageRevision) " + stateDescription(for: textView))
        isApplyingExternalState = false
    }
    ...
    private func projectedDraft(from textView: UITextView) -> MemoryCardEditorDraft {
        let items = projectedItems(from: textView)
        return MemoryCardEditorDraft(items: items.isEmpty ? [.text("")] : items)
    }
    ...
    private func projectedItems(
        ...
        textView.textStorage.enumerateAttributes(in: range) { attributes, subrange, _ in
            if let attachment = attributes[.attachment] as? MemoryCardTextKitModuleAttachment {
    ...
    private func attributedString(
        for items: [MemoryCardContentItem]
    ) -> NSAttributedString {
        let result = NSMutableAttributedString()
    ...
    private func attributedAttachment(
        ...
        let result = NSMutableAttributedString(
            attachment: MemoryCardTextKitModuleAttachment(
                item: item,
                leadingAdvance: MemoryCardEditorCapsuleMetrics.attachmentLeadingAdvance,
                trailingAdvance: MemoryCardEditorCapsuleMetrics.attachmentTrailingAdvance
            )
        )
    ...
    private func structuredPasteboardData() -> Data? {
        UIPasteboard.general.data(
            forPasteboardType: MemoryCardEditorPasteboard.structuredType
        ) ?? UIPasteboard.general.data(
            forPasteboardType: MemoryCardEditorPasteboard.legacyStructuredType
        )
    }
    ...
    private func registerUndoSnapshot(
        ...
        let snapshot = MemoryCardTextKitUndoSnapshot(
    ...
    private func restoreUndoSnapshot(
        _ snapshot: MemoryCardTextKitUndoSnapshot,
    ...
    private func editingAttributes() -> [NSAttributedString.Key: Any] {
        ...
        attributes[.baselineOffset] =
            MemoryCardEditorInputMetrics.textBaselineOffset(for: font)
        return attributes
    }
    ...
    private func lineBoxAttributes(
        ...
        let lineHeight = MemoryCardEditorInputMetrics.lineHeight(for: font)
    ...
    private func refreshTextViewLayout(_ textView: UITextView) {
        guard let textView = textView as? MemoryCardTextKitTextView else { return }
    ...
    private func hasSameEditingContent(
        _ lhs: MemoryCardEditorDraft,
        _ rhs: MemoryCardEditorDraft
    ) -> Bool {
...
struct MemoryCardTextKitSessionEditor: View {
    ...
    init(
        ...
        self.onDraftChange = onDraftChange
        _session = State(
            initialValue: MemoryCardTextKitEditorSession(
                draft: draft,
                region: region,
    ...
    var body: some View {
        ...
        .padding(.vertical, MemoryCardEditorInputMetrics.rowVerticalPadding)
    }
}

private struct MemoryCardTextKitSessionRepresentable: UIViewRepresentable {
    let session: MemoryCardTextKitEditorSession
    let region: CardRegion
    let draft: MemoryCardEditorDraft
    let commandBus: MemoryCardTextKitCommandBus
    let onFocus: () -> Void
...
private final class MemoryCardTextKitTextView: UITextView {
    ...
    func updateVerticalTextAlignment() {
        ...
        let fontLineHeight = ceil(
            font?.lineHeight ?? MemoryCardEditorInputMetrics.fallbackLineHeight
        )
        ...
        let lineHeight = max(
            MemoryCardEditorInputMetrics.moduleAttachmentHeight,
            fontLineHeight
        )
    ...
    override func caretRect(for position: UITextPosition) -> CGRect {
        ...
        let caretWidth = MemoryCardEditorInputMetrics.caretWidth
        let caretHeight = MemoryCardEditorInputMetrics.caretHeight
...
private extension MemoryCardTextKitEditorSession {
