...
struct MemoMarkiOSV1View: View {
    ...
    private var presentationStyleBinding:
    ...
    private func switchPresentationStyle(
        ...
        regionDraftsByPresentationStyle[presentationStyle] = regionDrafts
        presentationStyle = newStyle
        regionDrafts = regionDraftsByPresentationStyle[newStyle]
            ?? defaultRegionDrafts()
    ...
    private var regionDraftsForSaving:
        [RecordCardPresentationStyle: [CardRegion: V1EditorDraft]] {
        var result = regionDraftsByPresentationStyle
        result[presentationStyle] = regionDrafts
        return result
    }
    ...
    private var configurationOptionList: some View {
        ...
        return V1ConfigurationOptionList(
            ...
            output: V1ConfigurationOutputBindings(
                outputTarget:
                    $outputDraftState.outputTarget,
                availableAlbums:
                    outputDraftState.availableAlbums,
    ...
    @MainActor
    private func saveCurrentV1ConfigurationSnapshot()
    async -> BatchConfigurationSnapshot? {
        guard await applyCurrentV1Configuration() else {
            return nil
        }

        // The apply coordinator has completed the durable configuration write
        // before returning. Rebuild from that durable source and pass this
        // value into the intake request so picker latency cannot observe a
        // later mutable UI configuration.
        let snapshot = SettingsService()
    ...
    private func loadAlbumOptions() async {
        outputDraftState.albumLoadGeneration &+= 1
        let request = V1OutputAlbumLoadRequest(
            generation: outputDraftState.albumLoadGeneration,
            subjectID: session.state.selectedSubject?.id,
        ...
        let result = await V1PhotoProcessingQuickActionCoordinator
            .processPickedPhotoItems(
                saveCurrentConfiguration:
                    saveCurrentV1ConfigurationSnapshot,
    ...
    private func expansionBinding(
        for section: MemoMarkiOSV1EntrySection
    ) -> Binding<Bool> {
        Binding(
            get: {
