...
struct V1ContentItem: Identifiable, Hashable, Codable {
    ...
    var copyingForInsertion: V1ContentItem {
        V1ContentItem(
            id: UUID(),
            sourceItemID: nil,
            kind: kind,
...
struct V1EditorClipboardPayload: Codable, Hashable {
    static let schemaVersion = 1

    let schema: Int
    let items: [V1ContentItem]

    init(
        items: [V1ContentItem],
        schema: Int = V1EditorClipboardPayload.schemaVersion
    ) {
        self.schema = schema
        self.items = items
    }
    ...
    var displayText: String {
        V1EditorDraft(items: items).singleLineText
    }
}

enum V1EditorClipboardCodec {
    static func encode(
        _ payload: V1EditorClipboardPayload
    ) -> Data? {
    ...
    static func decode(
        _ data: Data
    ) -> V1EditorClipboardPayload? {
        guard let payload = try? JSONDecoder().decode(
            V1EditorClipboardPayload.self,
            from: data
        ),
        payload.schema == V1EditorClipboardPayload.schemaVersion,
