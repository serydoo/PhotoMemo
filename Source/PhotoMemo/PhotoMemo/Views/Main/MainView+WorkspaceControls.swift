...
enum MainOperationGuideTopic:
    ...
    fileprivate var introduction: String {
        ...
        case .overview:
            return "时光记的主界面不是批量工作台，而是日常分享前的准备中心。左侧负责默认风格、记忆日期、自定义区域和输出规则，右侧负责实时预览。"

        case .configurations:
            return "三个默认风格一次只会使用一个。切换时，排版、文案和底部展示会一起刷新；保存时会把当前风格内容整体写回当前选中的风格。"

        case .composer:
            return "四个自定义区域是时光记最核心的组合层。无论输入自定义文字还是插入照片信息、智能模块，都必须先明确选中区域，再按光标位置组合内容。"
        ...
        case .output:
            return "时光记会生成一张新图，并按当前本地链路把它写回系统图库。输出区的重点是选对目标相册，并让处理后的图片继续保留尽可能完整的原图元数据。"
        }
    }
            ...
            return [
                MainOperationGuideSection(
                    ...
                    items: [
                        ...
                        "如果不手动指定现有相册，系统会自动创建或复用时光记相册。",
                        "时光记的目标是在生成一张新图的同时，尽量保留原图拍摄时间与可保留信息。"
                    ]
                ),
                MainOperationGuideSection(
                    title: "使用建议",
                    items: [
                        ...
                        "时光记会生成一张新图，而不是修改原图，所以原照片仍会留在系统图库里。",
