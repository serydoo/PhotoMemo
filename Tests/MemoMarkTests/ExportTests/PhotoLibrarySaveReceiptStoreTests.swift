...
struct PhotoLibrarySaveReceiptStoreTests {
    ...
    @Test("a confirmed PhotoKit transaction failure clears its pending save")
    func confirmedTransactionFailureCanBeRetried() {
        let policy =
            PhotoLibrarySaveTransactionFailurePolicy()
    ...
    @Test("submitted intent survives receipt write failure so retry cannot duplicate")
    func submittedIntentSurvivesReceiptWriteFailure() throws {
        let suiteName =
            "PhotoLibrarySaveReceiptStoreTests.AmbiguousWriteFailure.\(UUID().uuidString)"
        let defaults = try #require(
            UserDefaults(suiteName: suiteName)
        )
        defaults.removePersistentDomain(forName: suiteName)
        ...
        #expect(
            store.recordIntent(
                for: "task/ambiguous-write-failure"
            )
        )
        ...
        #expect(
            failingStore.recordIntentAssetIdentifier(
                "asset-created-before-receipt-failure",
                for: "task/ambiguous-write-failure"
            ) == false
        )
        #expect(
            failingStore.hasPendingIntent(
                for: "task/ambiguous-write-failure"
            )
        )
        #expect(
    ...
    @Test("PhotoKit saves persist only a pending intent inside each transaction")
    func pendingIntentIsRecordedInsideBothPhotoKitChangeTransactions() throws {
        let exportSource = try sourceText(
            "Source/MemoMark/MemoMark/Services/PhotoLibraryExportService.swift"
        )
        let livePhotoSource = try sourceText(
