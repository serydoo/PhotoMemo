...
struct LivePhotoBatchQueueExecutionTests {
    ...
    func preparedLivePhotoBundlesResolveStillAndPairedVideoResources() throws {
        let rootURL =
            ...
            .appendingPathComponent(
                "MemoMark.PreparedLivePhotoBundle.\(UUID().uuidString)",
                isDirectory: true
            )
        let bundleURL =
            rootURL
    ...
    func preparedLivePhotoBundlesRejectIncompleteOrNonDirectorySources() throws {
        let rootURL =
            ...
            .temporaryDirectory
            .appendingPathComponent(
                "MemoMark.PreparedLivePhotoBundleReject.\(UUID().uuidString)",
                isDirectory: true
            )
        defer {
            try? FileManager.default
    ...
    func preparedLivePhotoBundlesRejectAmbiguousOrMismatchedPairs() throws {
        let rootURL =
            ...
            .temporaryDirectory
            .appendingPathComponent(
                "MemoMark.PreparedLivePhotoBundleAmbiguous.\(UUID().uuidString)",
                isDirectory: true
            )
        defer {
            try? FileManager.default
    ...
    func livePhotoPayloadsUseInternalLivePhotoProcessor() async throws {
        let suiteName =
            "MemoMark.LivePhotoBatchQueueExecutionTests.\(UUID().uuidString)"
        let defaults =
            try #require(
                UserDefaults(
                    suiteName: suiteName
    ...
    func staticLivePhotoOutputFailsWithoutRecoverableStillResource() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "MemoMark.StaticLivePhotoAdmission.\(UUID().uuidString)",
                isDirectory: true
            )
        defer {
    ...
    func realLivePhotoProcessorUsesSourceBundleResourcesWithoutPhotoKitLoading() async throws {
        let rootURL =
            ...
            .temporaryDirectory
            .appendingPathComponent(
                "MemoMark.RealLivePhotoSourceBundle.\(UUID().uuidString)",
                isDirectory: true
            )
        let bundleURL =
            rootURL
