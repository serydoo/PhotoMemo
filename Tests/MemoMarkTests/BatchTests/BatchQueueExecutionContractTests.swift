...
struct BatchQueueExecutionContractTests {
        ...
        let source = try String(
            contentsOf:
                ...
                .appendingPathComponent(
                    "Source/MemoMark/MemoMark/Repositories/PhotoRepository.swift"
                ),
            encoding: .utf8
        )
    ...
    func taskProcessorContextKeepsExecutionInputs() {
        ...
        #expect(!context.route.usesLivePhotoProcessing)
    ...
    func retryableManagedSourceFailurePreservesSource() throws {
        let suiteName =
            "MemoMark.BatchQueueExecutionContractTests.Managed.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        let intakeDirectoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(suiteName, isDirectory: true)
    ...
    func finalNotificationMarkerGatesRepeatedDeliveryAttempts() async throws {
        ...
        let suiteName =
            "MemoMark.BatchQueueExecutionContractTests.Final.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        let job = BatchJob(
            title: "Completed",
            state: .completed,
...
private extension BatchQueueExecutionContractTests {
    ...
    func makeManagedSourceContext() throws -> ManagedSourceContext {
        let suiteName =
            "MemoMark.BatchQueueExecutionContractTests.Resources.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defaults.removePersistentDomain(forName: suiteName)
    ...
    @MainActor
    func processingOwnerStops(in store: BatchQueueStore) async throws {
        let clock = ContinuousClock()
        let deadline = clock.now.advanced(by: .seconds(5))
        ...
        Issue.record("Timed out waiting five seconds for the processing owner to stop")
        throw ContractTestTimeout.processingOwner
    }
