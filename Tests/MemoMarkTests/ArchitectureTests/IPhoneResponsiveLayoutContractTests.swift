...
struct IPhoneResponsiveLayoutContractTests {
    ...
    func rootDelegatesSystemPresentationAndRuntimeCoordination() throws {
        ...
        // Keep a bounded root budget while allowing native picker transactions,
    ...
    func cardEditorVisualContinuityPreservesStableInteractionSkeleton() throws {
        ...
        #expect(cluster.contains("photoDescriptionRegions"))
        #expect(cluster.contains("if !photoDescriptionRegions.isEmpty"))
    ...
    func emptyCardEditorCaretKeepsStableTextMetrics() throws {
        ...
        #expect(textKit.contains("static let titleColumnWidth: CGFloat = 96"))
        #expect(textKit.contains("static let multiRegionTitleColumnWidth: CGFloat = 48"))
    ...
    func cardEditorDeletesAttachmentRelativeToCaret() throws {
        ...
        #expect(textKit.contains("let precedingLocation = caretLocation - 1"))
        #expect(textKit.contains(".attachment,"))
        #expect(textKit.contains("at: precedingLocation"))
        #expect(
            textKit.contains(
                "NSRange(location: precedingLocation, length: 1)"
            )
        )
        #expect(textKit.contains("let restoredCaretLocation = precedingLocation"))
        #expect(
            !textKit.contains(
                "attribute(.attachment, at: documentLength - 1, effectiveRange: nil)"
            )
        )
        #expect(
            !textKit.contains(
                "NSRange(location: documentLength - 1, length: 1)"
            )
        )
    }
    ...
    func cardEditorKeepsModulesStructuredAcrossCopyAndPaste() throws {
        ...
        #expect(textKit.contains("onCopy"))
        #expect(textKit.contains("onPaste"))
        #expect(textKit.contains("override func copy(_ sender: Any?)"))
        #expect(textKit.contains("override func paste(_ sender: Any?)"))
        #expect(textKit.contains("copySelection(in:"))
        #expect(textKit.contains("pasteStructuredContent(in:"))
        #expect(textKit.contains("com.serydoo.MemoMark.editor-content"))
        #expect(textKit.contains("legacyStructuredType"))
        #expect(textKit.contains("registerUndoSnapshot"))
    ...
    func cardEditorRequestsRealTextKitFocus() throws {
        ...
        #expect(textKit.contains("textView.isFirstResponder"))
        #expect(textKit.contains("let commandSelection"))
        #expect(textKit.contains("? textView.selectedRange"))
        #expect(textKit.contains(": selectedRange"))
    ...
    func cardContentEditorDoesNotRenderDuplicateRightSideRowPreviews() throws {
        ...
        #expect(cluster.contains("修改会实时出现在上方完整卡片预览中。"))
        #expect(cluster.contains("这里的内容还会写入 Apple Photos 的照片说明，方便之后查找。"))
        #expect(!cluster.contains("点“完成”返回配置中心；收起键盘不会离开编辑页"))
    }
    ...
    func outputAndProcessingPreserveAccessibilityContent() throws {
        ...
        #expect(processing.contains("dynamicTypeSize.isAccessibilitySize ? 3 : 2"))
    }
    ...
    func subjectAnchorModulesUseCompactSemanticColorTypeLabels() throws {
        ...
        #expect(module.contains("accessibility.time_anchor_type_prefix"))
    ...
    func subjectEditorKeepsStableExpressionTitleAndBoundedDeletion() throws {
        ...
        #expect(deleteButton.contains(".contentShape(Rectangle())"))
    ...
    func iPhoneViewsAvoidPhysicalScreenAndDeviceModelBranching() throws {
        let viewsDirectory = repositoryRoot
            .appendingPathComponent(
                "Source/MemoMark/MemoMark/iOS/Views",
                isDirectory: true
            )
    ...
    @Test("card content editing and photo description follow the style contract")
    func cardContentEditingAndPhotoDescriptionFollowStyleContract() throws {
        let recordCard = try sourceText(
            "Source/MemoMark/MemoMark/Models/RecordCard.swift"
        )
