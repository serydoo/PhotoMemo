...
struct SubjectOverviewPresenterTests {
    ...
    func configurationFlowCommitsFallbackAnchorAfterDeletingCurrentAnchor() async throws {
        ...
        let flow = try #require(
            SubjectConfigurationFlowState(liveSession: liveSession)
        )
    ...
    func subjectOverviewSaveWritesLatestSubjectToDurableAggregate() throws {
        ...
        let combinedSource = source + actionsSource

        #expect(combinedSource.contains("persistCurrentSubjectChanges"))
        #expect(combinedSource.contains("LocalConfigurationLibraryPresenter"))
        #expect(combinedSource.contains(".updatingSubject("))
        #expect(combinedSource.contains(".saveConfigurationLibrary(candidate)"))
        #expect(combinedSource.contains(".updateConfigurationLibraryReference("))
        #expect(
    ...
    func subjectOverviewKeepsCompactCurrentIdentityActionsAndUsesDisplayName() throws {
        let railSource = try sourceText(
            "Source/MemoMark/MemoMark/iOS/Views/SubjectOverviewRailSurface.swift"
        )
        ...
        #expect(railSource.contains("SubjectOverviewSubjectRail"))
    ...
    func basicInformationUsesContinuousContactsStyleForm() throws {
        ...
        #expect(normalizedField.contains("accessibility.clear_format"))
    ...
    func subjectEditorCentersNativeAvatarActionsAboveForm() throws {
        ...
        let avatarSurfaceSource = try sourceText(
        ...
        #expect(overviewSource.contains("SubjectAvatarContactEditorSurface("))
        #expect(avatarSurfaceSource.contains("PhotosPicker("))
        #expect(
            avatarSurfaceSource.contains(
                "avatarIsAvailable\n                                ? \"编辑\""
            )
        )
        #expect(avatarSurfaceSource.contains("Image(systemName: \"minus\")"))
        #expect(overviewSource.contains("onRemove: removeAvatarFromDraft"))
        #expect(avatarSurfaceSource.contains("accessibility.delete_avatar"))
        #expect(avatarSurfaceSource.contains(".disabled(isOptimizing)"))
    ...
    func subjectOverviewFactsUseContactStyleHorizontalRows() throws {
        ...
        let factRow = try sourceSection(
            in: overviewSource,
            from: "private struct SubjectOverviewFactRow",
            to: "#endif"
        )
