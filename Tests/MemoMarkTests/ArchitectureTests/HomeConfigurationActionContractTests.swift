...
struct HomeConfigurationActionContractTests {
    ...
    func homeMakesApplePhotosSharingPrimary() throws {
        ...
        #expect(source.contains("WelcomePresentation.workflowSteps"))
        ...
        #expect(source.contains("CompactPrimaryActionButtonStyle()"))
        #expect(!source.contains("applePhotosEntryIcon"))
        #expect(pagesSource.contains("entryFlowState.showsWorkflowGuide = true"))
        #expect(presentationSource.contains("WorkflowGuideSurface("))
    ...
    func homePhotoPickerUsesFirstUseBoundary() throws {
        ...
        let containerSource = try sourceText(
    ...
    @Test("primary navigation labels resolve through interface localization")
    func primaryNavigationLabelsUseInterfaceLocalization() throws {
        let navigationSource = try sourceText(
            "Source/MemoMark/MemoMark/iOS/Views/AdaptiveNavigationShell.swift"
        )

        #expect(navigationSource.contains("localized(\"首页\")"))
        #expect(navigationSource.contains("localized(\"配置\")"))
        #expect(navigationSource.contains("localized(\"进展\")"))
        #expect(navigationSource.contains("destination.localizedTitle"))
    }

    @Test("Apple Photos first-use state is marked for both intake representations")
    func applePhotosFirstUseStateIsMarkedForBothIntakeRepresentations() throws {
        let source = try sourceText(
            "Source/MemoMark/MemoMark/App/ExternalPhotoIntakeCenter.swift"
        )
        ...
        #expect(
            source.components(
                separatedBy: "markApplePhotosShareUsageIfNeeded(for: source)"
            ).count >= 3
        )
    }
    ...
    func homeKeepsWorkflowReminderBelowPresets() throws {
        ...
        #expect(source.contains("private struct HomeWorkflowReminderCard"))
        ...
        let cardSource = try #require(
            source.components(
                separatedBy: "private struct HomeWorkflowReminderCard"
            ).last
        )
    ...
    func configurationActionsUseOverflowMenu() throws {
        ...
        #expect(!source.contains("HomeConfigurationSwipePresenter"))
    ...
    func cardHeaderActionsShareTheNativeIconButtonTreatment() throws {
        ...
        let recentHistorySource = try sourceText(
            "Source/MemoMark/MemoMark/iOS/Views/TaskRecentHistorySurface.swift"
        )

        #expect(supportSource.contains("struct ConfigurationCardHeaderIconButton"))
        ...
        #expect(taskSource.contains("TaskRecentHistorySurface("))
        #expect(recentHistorySource.contains("systemImage: \"ellipsis\""))
        #expect(recentHistorySource.contains("if rows.count > inlineRows.count"))
    }
    ...
    func homeSurfacesConfigurationActionFeedback() throws {
        ...
        let actionSource = try sourceText(
            "Source/MemoMark/MemoMark/iOS/Views/ConfigurationLibraryActions.swift"
        )
        ...
        #expect((rootSource + runtimeSource).contains("presentHomeConfigurationActionFeedback"))
        #expect((rootSource + runtimeSource).contains("configurationLibraryActions.decide"))
    ...
    func renameAndSaveCallbacksUseExplicitActionDecisions() throws {
        ...
        let source = rootSource + pagesSource + actionsSource
...
private extension HomeConfigurationActionContractTests {
