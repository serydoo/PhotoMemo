...
struct HomeMemoryPresetRowSurfaceContractTests {
    ...
    func presetRowStaysOutsideTheHomeCoordinator() throws {
        ...
        #expect(rowSource.contains("Button(action: onSelect)"))
        #expect(rowSource.contains(".buttonStyle(.plain)"))
        #expect(rowSource.contains(".isSelected"))
        #expect(!rowSource.contains(".onTapGesture(perform: onSelect)"))
