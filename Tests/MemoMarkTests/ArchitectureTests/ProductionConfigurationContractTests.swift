...
struct ProductionConfigurationContractTests {
    ...
    func productionIdentitySurvivesEnvironmentRestart() async throws {
        let fixture = try Self.makeFixture()
        let suiteName =
            "MemoMark.ProductionConfigurationRestart.\(UUID().uuidString)"
    ...
    @MainActor
    @Test("drained versioned request recovers a valid frozen snapshot after durable revision advances")
    func drainedVersionedRequestRecoversFrozenSnapshotAfterDurableRevisionAdvances() async throws {
        let fixture = try Self.makeFixture()
        let context = try Self.makeEnvironment()
        defer { Self.cleanup(context) }

        let initialReceipt = try await context.environment.coordinators
            .configuration
            .saveConfigurationLibrary(fixture.aggregate)
        let frozenSnapshot = try ProductionConfigurationSnapshotFactory
            .resolve(
                reference: ProductionConfigurationReference(
                    configurationID: fixture.configuration.id,
                    revision: fixture.configuration.revision
        ...
        var advancedAggregate = fixture.aggregate
        advancedAggregate.revision = initialReceipt.revision
        advancedAggregate.subjects[0].configurations[0].revision =
            fixture.configuration.revision + 1
        _ = try await context.environment.coordinators.configuration
            .saveConfigurationLibrary(advancedAggregate)
        ...
        #expect(
            ...
            == frozenSnapshot.canonicalProductionSnapshot
        )

        let requestEvents = MemoMarkShareDiagnostics.loadEvents(
            defaults: context.defaults
        ).filter {
            $0.requestID == request.id
        }
        #expect(
            requestEvents.contains {
                $0.stage
                == .configurationCompatibilityRecovery
                && $0.message.contains(
                    "usingFrozenRequestSnapshot=true"
                )
            }
        )
        #expect(
    ...
    func legacyVersionedRequestKeepsFrozenTransportWhenCanonicalSnapshotIsMissing() async throws {
        ...
        let legacyTransport = BatchConfigurationSnapshot(
            configurationID: fixture.configuration.id,
            configurationRevision: fixture.configuration.revision,
            productionContractVersion:
                ProductionConfigurationReference
            ...
            shouldWritePhotoDescription: false,
            photoDescriptionOverride: "",
            selectedAlbumIdentifier: "legacy-album"
        )
        let request = ExternalPhotoIntakeRequest(
            launchSource: .shareExtension,
            urls: [sourceURL],
    ...
    private static func makeEnvironment() throws -> EnvironmentContext {
        let suiteName =
            "MemoMark.ProductionConfigurationContractTests.\(UUID().uuidString)"
        let defaults = try #require(
            UserDefaults(suiteName: suiteName)
        )
        defaults.removePersistentDomain(forName: suiteName)
