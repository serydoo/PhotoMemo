...
struct ConfigurationCenterInitialLibraryTests {
    ...
    func freshInstallationDefaultsHaveCanonicalConfigurationRecords() throws {
        let suiteName =
            "PhotoMemo.ConfigurationCenterInitialLibraryTests.fresh.\(UUID().uuidString)"
    ...
    func freshInstallationDefaultsCanEnterDurableDeletionFlow() throws {
        let suiteName =
            "PhotoMemo.ConfigurationCenterInitialLibraryTests.delete.\(UUID().uuidString)"
        let defaults = try #require(
            UserDefaults(suiteName: suiteName)
        )
        defaults.removePersistentDomain(forName: suiteName)
    ...
    func legacyInstallationDefaultsMigrateToCanonicalConfigurationLibrary() throws {
        let suiteName =
            "PhotoMemo.ConfigurationCenterInitialLibraryTests.\(UUID().uuidString)"
        let defaults = try #require(
            UserDefaults(suiteName: suiteName)
        )
        defaults.removePersistentDomain(forName: suiteName)
    ...
    func corruptedCanonicalStorageDoesNotSynthesizeDefaults() throws {
        let suiteName =
            "PhotoMemo.ConfigurationCenterInitialLibraryTests.corrupt.\(UUID().uuidString)"
        let defaults = try #require(
            UserDefaults(suiteName: suiteName)
        )
        defaults.removePersistentDomain(forName: suiteName)
    ...
    func corruptedCanonicalStorageDoesNotSynthesizeLegacyLibrary() throws {
        let suiteName =
            "PhotoMemo.ConfigurationCenterInitialLibraryTests.corruptLegacy.\(UUID().uuidString)"
        let defaults = try #require(
            UserDefaults(suiteName: suiteName)
        )
        defaults.removePersistentDomain(forName: suiteName)
