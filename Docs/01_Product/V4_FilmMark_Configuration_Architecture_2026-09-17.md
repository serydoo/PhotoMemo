...
本节中“默认横向预览等高”及“FM 默认仅展示照片底部局部”的规则，由
2026-09-23 产品决定更新；下文保留原始决策记录供追溯。
...
### 2026-09-23：样式区分与照片方向预览

状态：产品负责人已接受预览分流方向；具体预览高度与横/竖样片比例等待本轮视觉评审和实体设备观察后确定。

- 撤销“Classic White、Minimal、FilmMark 默认横向预览区等高”的要求。高度服务于各样式需要表达的信息，不为等高而裁切照片或压缩预览内容。
- Classic White 的预览展示完整信息卡，不显示源照片。
- Minimal 与 FilmMark 的默认预览展示完整示例照片及成品元素相对照片的位置。横图/竖图通过同一预览舞台左右切换，并提供可见方向标签或等效原生控制；切换是纯预览状态。
- 横向与竖向各使用一张匹配方向的本地示例照片。图片只用于比较样式在不同画幅中的相对位置，不进入配置、输出或照片库。
- 示例照片应保持完整可辨，不因填满固定比例容器而裁切；舞台高度与照片适配关系由 iPhone 原型评审确定，再形成可测量规则。
- FM 展开“位置与字号”时，同一顶部预览切换到精细校准画布；画布以 Layout Engine 的完整画布比例显示锚点、安全区、文字框与字号。收起后返回整体内容预览，不新增第二个预览权威。
- 横/竖样片和预览尺寸不得进入持久化配置、生产快照、Renderer 或导出载荷。照片舞台仍读取同一 resolved presentation，不得在 View 中复制 FM 几何。
