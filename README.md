...
当前本地源码候选版本：**MemoMark 2.2.2（构建 95）**。本次正式版从 2026-08-24（周一）约 06:54 的推送波次开始计算，并对齐最近已推送提交 `c0f11a7`（06:49:40）；上一轮 TestFlight 已交付的用户功能与修复均纳入本次正式版。当前尚未提交 App Store。详见 [2.2.2（95）版本更新说明](Docs/07_Releases/2026-08-27-2.2.2-release-notes.md)。
...
发布与同步遵循 [MemoMark 发布同步规范](Docs/07_Releases/RELEASE_SYNC_STANDARD.md)。版本号、更新范围、GitHub 同步、TestFlight 和 App Store 提交分别确认，不把源码检查点等同于正式发布。当前本地候选为 MemoMark `2.2.2`（构建 `95`），正式版同步范围为 `c0f11a7..HEAD`；上一版 TestFlight 内容已纳入本候选，尚未提交 App Store。
