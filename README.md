...
当前本地源码候选版本：**MemoMark 2.3.2（构建 108）**。本轮按用户指定从 2.3.1 版本更新起点整理，重点继续推进 FM（FilmMark／胶片时间）开发，同时修复预设样式选择与实际输出不一致，并统一配置中心的文字层级和自适应选择反馈。当前继续维护 MemoMark Commerce v1.1：历史永久权益与 48 元激活码权益继续保留，新用户使用 MemoMark+ 月度或年度订阅，订阅一次解锁全部内置的记忆表达方式；主页选图入口累计使用 10 次后显示 24 小时的日常使用引导，24 小时后自动收起。当前状态为 `Version Locked; Release Evidence Open`。详见 [2.3.2（108）版本更新说明](Docs/07_Releases/2026-09-21-2.3.2-release-notes.md)。
