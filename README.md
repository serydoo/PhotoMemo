...
当前本地源码候选版本：**MemoMark 2.2.2（构建 95）**。本轮从最近一次已推送的 GitHub 提交 `03a49f1` 开始计算，修复 Xcode Cloud 归档在旧版 Photos SDK 下的兼容性问题；尚未上传 TestFlight 或提交 App Store。详见 [2.2.2（95）版本更新说明](Docs/07_Releases/2026-08-27-2.2.2-release-notes.md)。
...
发布与同步遵循 [MemoMark 发布同步规范](Docs/07_Releases/RELEASE_SYNC_STANDARD.md)。版本号、更新范围、GitHub 同步、TestFlight 和 App Store 提交分别确认，不把源码检查点等同于正式发布。当前本地候选为 MemoMark `2.2.2`（构建 `95`），同步范围为 `03a49f1..当前工作区`；本轮已获得明确授权同步 GitHub，未上传 TestFlight 或提交 App Store。
