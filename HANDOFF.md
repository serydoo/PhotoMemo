...
- 已完成静态检查、聚焦几何/编辑器契约验证，以及此前记录的 macOS、通用 iOS、Share、Widget、Device QA 构建证据；本轮 91 号构建会在提交前重新核验。
...
- 构建号更新与本地 `main` 合并已完成；合并提交为 `62e6775`。生成的 Xcode Cloud manifest、xcuserdata、`.DS_Store` 和脚本缓存仅作为可恢复本地清理对象，私人研究、设备数据、DerivedData 和诊断产物继续保留。
...
- iOS、macOS、Share Extension、Widget Extension 和测试 target 的版本字段已统一为 `2.1.3 (86)`；合并后的四个 scheme 已重新构建并核验产物版本。
- 上一轮设备中仍保留 `2.1.3 (85)`，本轮尚未重新安装 86；不卸载、不清空设备数据。
- ProductionDiagnosticEvent 与通知深链的 actor-isolation 修复已通过 focused tests 和合并后四个 scheme 构建；相关 warning 扫描为 0。后续只剩远程推送、Xcode Cloud 新构建、TestFlight/App Store 外部交付和用户决定的发布验收。
...
## 2026-07-17 首页说明关闭与对象头像保存修复

- 首页开发背景说明支持本机持久关闭；关闭按钮使用系统 `xmark`、现有卡片语义颜色、28 pt 圆形视觉和 44 pt 触控区域。
- 完整产品缘起保留在设置“为什么是时光记”中，首页关闭后仍可重新查看。
- 配置保存会把运行时头像绝对路径转换为可持久化相对路径，并同步显示、Logo、预览三类资源清单。
- 配置恢复和切换会重新解析运行时头像路径；生产快照在 `subjectAvatar` Logo 模式下直接读取已保存的对象头像。
- 头像回归测试完成红灯/绿灯；配置、生产快照、首页和设置共 36 项聚焦测试通过；`git diff --check` 通过。
- 最新 `PhotoMemoiOS` 已完成签名真机构建，覆盖安装并成功启动到 `iPhone7`，未清除原有应用数据。
- 当前不提交、不推送；等待后续真机反馈一起修复后，再统一同步 GitHub。BrandMark 研究内容继续独立保留，不纳入该批次。

## 2026-07-17 对象栏层级与左滑操作优化

- 记忆对象顶部栏调整为“当前使用、对象名称、保存图标、切换对象”，移除与下方编辑区重复的头像和关系备注；窄屏使用 `ViewThatFits` 自动回退布局。
- 主体名称改为明显可点击的强调色菜单，并将“当前展示”结果和关系备注拆为清晰的主次层级。
- 时间锚点标准行高调整为 64 pt；辅助功能大字号解除列表固定总高度，单行保持最小 64 pt，避免文字受限。
- 时间锚点与首页“我的配置”均关闭整行快速滑动；继续使用系统左滑操作和通用图标，删除先进入确认框，不恢复曾导致崩溃的嵌套 `List` 或 destructive 滑动按钮。
- 16 项聚焦架构测试通过，iOS Simulator Debug 构建通过，`git diff --check` 通过。
- 最新 `PhotoMemoiOS` 已完成签名真机构建，覆盖安装并成功启动到 `iPhone7`，未清除现有应用数据；左滑手感、长名称与连续操作仍需真机人工确认。
- 当前不提交、不推送；待真机反馈后再与其他非 BrandMark 改动统一整理并同步 GitHub。

## 2026-07-18 记忆对象与时间锚点原生交互收口

- 记忆对象顶部“当前使用”和对象切换列表统一显示稳定的对象名称，不再优先使用昵称；头像和关系备注不再在顶部重复展示。
- 对象编辑区将“表达称呼”改为整行可点击菜单，菜单展示每个来源的实际内容，空来源不可选择；基础资料统一为“对象名称、昵称、与我的关系、专属称呼”四行原生分隔布局。
- 表达预览与生产输出共用 MemorySubject 解析器；解析结果同时返回实际命中的来源，选中字段被清空并回退时，界面的“来自”说明不会继续显示错误来源。
- 时间锚点二级页移除外层 ScrollView 和嵌套卡片，改为唯一的根级系统 List，使 swipeActions 获得真实列表行上下文；继续关闭整行快速滑动，滑动删除只打开确认框，实际删除仍在 destructive 确认按钮中执行。
- 锚点行用类型图标替代数字序号；日期滚轮由 52 pt 提升到 144 pt；编辑弹窗使用“新增时间锚点 / 编辑时间锚点”固定标题，以及“锚点日期、锚点类型、自定义锚点名称”字段文案和 70% 初始高度。
- 表达称呼、基础资料输入行和锚点行均增加辅助功能字号降级布局，避免窄屏和大字体下来源、标签、标题或类型互相挤压。
- TDD 红灯阶段确认 14 项中 4 项按旧实现失败；最终对象概览、表达解析、锚点事务、首页防嵌套列表等聚焦测试通过，git diff --check 通过，PhotoMemoiOS iOS Simulator Debug 构建通过。
- 最新 1.7 (7) arm64 开发签名包已覆盖安装到 iPhone7，未卸载且未清除应用数据；首次自动启动仅因设备锁屏被系统拒绝，解锁后需重试启动并人工检查左滑、日期滚轮、取消回滚和保存持久化。
- 当前继续保持不提交、不推送；BrandMark 研究内容仍独立保留，不纳入本轮真机检查批次。

## 2026-07-18 未插入位置模块时的显示方式预选回归修复

- 修复“卡片与布局”中未插入位置模块时无法选择位置显示方式的回归；摘要区、详情区和旧 V1 入口均不再按模块是否存在禁用选择控件。
- 位置显示方式恢复为独立配置：用户可先选择自动兼容、省份城市、城市区县、省市区县或经纬度，之后插入位置模块时自动采用已预选配置。
- 新旧入口选择后都通过 ConfigurationCoordinator 立即写入现有 SettingsRepository；离开页面或重启前不必先插入模块，也不会丢失预选值。
- 新 Configuration Center 在页面重新出现时刷新持久化位置配置，避免 SwiftUI 保留旧页面状态后继续使用陈旧预选值。
...
- 首页配置行和完整锚点行增加 `ellipsis.circle` 图标菜单，复用原保存、配置、删除回调与删除确认；专用根级 `List` 左滑和 `allowsFullSwipe: false` 保持不变。
- TDD 红灯确认旧实现缺少对象聚合更新 API；最终四组聚焦测试 34 项全部通过。
