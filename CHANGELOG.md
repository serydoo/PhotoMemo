...
- Consolidated the user-facing and internal release materials from the 2.3.0
  (105) candidate on September 7 through the current 2.3.3 (111) candidate;
  no independently verified 2.3.1 release candidate is invented.
- Made the three presentation frameworks—Classic White, Minimal, and FilmMark—
  clearer as distinct expression directions, with more room to customize FilmMark
  style, content, placement, type size, substrate, time, and location.
- Reorganized the Configuration Center relationship between Memory Objects,
  presets, Memory Card preview, saving, and the final memory photo; refined
  Memory Object details and Time Anchor ordering for formal-user workflows.
...
- Raised all App, Extension, Widget, Device QA, and test target configurations
  to marketing version `2.3.3` and build `111`.

For `2.3.3 (111)`, the product-owner decision initially closes the scoped
FilmMark full path on the paired iPhone 17 Pro Max together with `TX-001` and
`BP-001`; later user feedback may reopen or extend those checks. StoreKit
purchase and historical-lifetime compatibility code was not changed. This does
not authorize GitHub push, TestFlight upload, App Store Connect mutation, or
claim a global/permanent production certification; those remain separate
evidence and authorization classes.
